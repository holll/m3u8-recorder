...
func main() {
	cfg := mustLoadConfig()
	log.Printf("Config: DOWNLOAD_PATH=%s SPLIT_SECONDS=%d WEBUI_PORT=%s",
		cfg.DownloadPath, cfg.SplitSeconds, cfg.WebPort)

	rec := NewRecorder(cfg.DownloadPath, cfg.SplitSeconds)
	server := NewWebUI(rec)
